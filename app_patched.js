...
function buildReferralLink(refCode) {
  if (!refCode) return '';
  return `https://t.me/${TG_BOT_USERNAME}?start=ref_${refCode}`;
}
...
async function fetchProfileFromEdge() {
  ...
  const response = await fetch(TG_PROFILE_URL, {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(`tg_profile HTTP ${response.status}: ${txt}`);
  }

  const json = await response.json();
  // Function returns { ok:true, profile:Ellipsis } in our recommended implementation.
...
const modal = {
  ...
  openProfile() {
    const p = runtimeProfile || demoData.profile;

    // Support both demo profile shape and RPC shape
    ...
    const rate = Number(p.success_rate ?? (total > 0 ? Math.round((done / total) * 100) : 0));
    ...
    const refCode = p.ref_code ?? (typeof p.referralLink === 'string'
      ? (p.referralLink.split('ref_')[1] || '')
      : '');

    document.getElementById('profileReferralLink').value =
      p.referral_link ?? p.referralLink ?? buildReferralLink(refCode);
...
function initApp() {
  setTimeout(async () => {
    ...
    try {
      ...
      console.warn('Could not load profile from tg_profile, using demoData.profile.', e);
      runtimeProfile = demoData.profile;
    }
    ...
    dom.referralLink.value = p.referral_link ?? p.referralLink ?? buildReferralLink(refCode);
